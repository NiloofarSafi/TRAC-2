...
def find_f1_threshold(model, val_gen, test_gen, average='binary'):
    ...
    # Returns:
        F1 score for the given preprocessed_data and
...
def tune_trainable(model, loss_op, optim_op, train_gen, val_gen, test_gen,
    ...
    # Arguments:
        ...
        train: Training preprocessed_data, given as a tuple of (inputs, outputs)
        val: Validation preprocessed_data, given as a tuple of (inputs, outputs)
        test: Testing preprocessed_data, given as a tuple of (inputs, outputs)
        epoch_size: Number of samples in an epoch.
...
def evaluate_using_weighted_f1(model, test_gen, val_gen):
    ...
    """
    # Evaluate on test and val preprocessed_data
...
def evaluate_using_acc(model, test_gen):
    ...
    # Arguments:
        model: Model to be evaluated.
        test_gen: Testing preprocessed_data iterator (DataLoader)
...
def train_by_chain_thaw(model, train_gen, val_gen, loss_op, patience, nb_epochs, checkpoint_path,
    ...
    # Arguments:
        ...
        val_data: Validation preprocessed_data.
...
def fit_model(model, loss_op, optim_op, train_gen, val_gen, epochs,
    ...
    # Arguments:
        ...
        train_gen: Training preprocessed_data iterator (DataLoader)
        val_gen: Validation preprocessed_data iterator (DataLoader)
...
class DeepMojiBatchSampler(object):
    ...
    def __iter__(self):
        # Hand-off preprocessed_data using batch_size
