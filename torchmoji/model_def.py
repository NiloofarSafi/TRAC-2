...
class TorchMoji(nn.Module):
    ...
    def forward(self, input_seqs):
        ...
        if not isinstance(input_seqs, PackedSequence):
            ...
            # Pack sequence and work on preprocessed_data tensor to reduce embeddings/dropout computations
            packed_input = pack_padded_sequence(input_seqs, input_lengths.cpu().numpy(), batch_first=True, enforce_sorted=True)
            reorder_output = True
            # print(packed_input)
        else:
        ...
        # Update packed sequence preprocessed_data for RNN

        packed_input = PackedSequence(x, packed_input.batch_sizes)

        embedding_out, _ = pad_packed_sequence(packed_input, batch_first=True)
        ...
        # Update packed sequence preprocessed_data for attention layer
        ...
        # print(input_seqs)
        x, weighted, att_weights = self.attention_layer(input_seqs, input_lengths)
        ...
        if reorder_output:
            ...
            w_reordered = Variable(weighted.data.new(weighted.size()))
            w_reordered[perm_idx] = weighted
            weighted = w_reordered


            embedding_out_reorder = Variable(embedding_out.data.new(embedding_out.size()))
            embedding_out_reorder[perm_idx] = embedding_out
            embedding_out = embedding_out_reorder

            # att_reorder = Variable(attention_applied.preprocessed_data.new(attention_applied.size()))
            # att_reorder[perm_idx] = attention_applied
            # attention_applied = att_reorder

            att_w_reorder = Variable(att_weights.data.new(att_weights.size()))
            att_w_reorder[perm_idx] = att_weights
            att_weights = att_w_reorder
        ...
        if self.return_attention:
            return outputs, weighted, att_weights, embedding_out
        else:
            return outputs, weighted, embedding_out
